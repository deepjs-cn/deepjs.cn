  ...
  function _doSend(message, responseCallback) {
    ...
    if (isIphone) {
      window.webkit &&
        ...
        window.webkit.messageHandlers.msfJsBridge.postMessage(sendMessageQueue);
      // var handler = (window.webkit && window.webkit.messageHandlers) || {};
      // var postMessage =
      //   (handler.msfJsBridge && handler.msfJsBridge.postMessage) || {};
      // if (isFunction(postMessage)) {
      //   postMessage(sendMessageQueue);
      // } else {
      ...
      if (window.msfJsBridge && window.msfJsBridge.callNative) {
        window.msfJsBridge.callNative(JSON.stringify(sendMessageQueue));
      }
      // 不能写成下面这样，会报错
      // var callNative = window.msfJsBridge.callNative || {};
      // if (isFunction(callNative)) {
      // }
    }
